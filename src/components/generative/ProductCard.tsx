...
export default function ProductCard({
  ...
  return (
    ...
    >
      {/* Image */}
      <div className="aspect-[16/9] w-full relative overflow-hidden bg-surface-container-high/40 flex items-center justify-center">
        {item.image_url ? (
          <img
            alt={item.name}
          ...
          <div className="flex flex-col items-center justify-center text-on-surface-variant/40 p-4 text-center z-10">
            <span className="material-symbols-outlined text-3xl mb-1">shopping_bag</span>
            <span className="text-xs font-medium line-clamp-1 text-on-surface/80">{item.name}</span>
          </div>
        )}
